...
struct DashboardView: View {
    ...
    private var suggestionCard: some View {
        ...
        return HStack(spacing: 14) {
            Circle()
                .fill(QuantumTheme.lavender.opacity(0.25))
                .frame(width: 54, height: 54)
                .overlay(Image(systemName: "sun.max.fill").foregroundStyle(QuantumTheme.lavender))
                ...
                HStack {
                    ...
                    Button(action: onToggle) {
                        Capsule(style: .continuous)
                            .fill(device.state.isOn ? QuantumTheme.lavender : Color.white.opacity(0.12))
...
private extension View {
    func roomActionButton(active: Bool) -> some View {
        self
            ...
            .padding(.horizontal, 14)
            .padding(.vertical, 10)
            .background(active ? AnyShapeStyle(QuantumTheme.gradient) : AnyShapeStyle(Color.white.opacity(0.08)))
            .clipShape(Capsule(style: .continuous))
    }
}
