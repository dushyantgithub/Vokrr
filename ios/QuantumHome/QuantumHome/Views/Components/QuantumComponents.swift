...
struct AuroraBackground: View {
    ...
    var body: some View {
        ZStack {
            QuantumTheme.background
                .ignoresSafeArea()
            Circle()
                .fill(QuantumTheme.lavender.opacity(0.33))
                .frame(width: 280, height: 280)
                .blur(radius: 60)
                .offset(x: animate ? -120 : 90, y: animate ? -310 : -240)
            Circle()
                .fill(QuantumTheme.sky.opacity(0.28))
...
struct QuantumLogo: View {
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(QuantumTheme.gradient)
        ...
        Text(title)
            ...
            .background(
                Capsule(style: .continuous)
                    .fill(isActive ? AnyShapeStyle(QuantumTheme.gradient) : AnyShapeStyle(Color.white.opacity(0.06)))
            )
            .overlay(
                Capsule(style: .continuous)
...
struct JarvisBar: View {
    ...
    private var statusColor: Color {
        switch status {
        case "listening":
            return QuantumTheme.lavender
        case "processing":
            return QuantumTheme.sky
        case "done":
            return QuantumTheme.mint
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                Circle()
                    .fill((health?.homeAssistant.ok ?? false) ? QuantumTheme.mint : Color.orange)
                ...
                ZStack(alignment: .topTrailing) {
                    ...
                    if unreadCount > 0 {
                        Text(unreadCount > 9 ? "9+" : "\(unreadCount)")
                            ...
                            .padding(.vertical, 2)
                            .background(Capsule().fill(QuantumTheme.gradient))
                            .offset(x: 12, y: -8)
                    }
                }
...
struct NotificationsSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .background(QuantumTheme.background.ignoresSafeArea())
...
struct BottomTabBar: View {
    @Binding var selectedTab: QuantumHomeTab

    var body: some View {
        HStack {
            ForEach(QuantumHomeTab.allCases) { tab in
                Button {
                    selectedTab = tab
                } label: {
                    VStack(spacing: 8) {
                    ...
                    .foregroundStyle(selectedTab == tab ? Color.white : QuantumTheme.secondaryText)
    ...
    private func label(for tab: QuantumHomeTab) -> String {
        switch tab {
    ...
    private func icon(for tab: QuantumHomeTab) -> String {
        switch tab {
            ...
            VStack(spacing: 24) {
                HStack {
                    ...
                    Button {
                        ...
                        Image(systemName: device.state.isOn ? "power.circle.fill" : "power.circle")
                            .font(.system(size: 34))
                            .foregroundStyle(device.state.isOn ? QuantumTheme.lavender : Color.white.opacity(0.8))
                    }
                    .buttonStyle(.plain)
                }
                ...
                if device.supportsLevel {
                    VStack(spacing: 20) {
                        ...
                        .tint(QuantumTheme.lavender)
                    }
                    .padding(24)
                    .glassCard(tint: QuantumTheme.lavender)
                }

                if device.supportsColorTemperature {
                    HStack(spacing: 12) {
                        temperatureButton(title: "Warm", kelvin: 2700, tint: QuantumTheme.coral)
                        temperatureButton(title: "Neutral", kelvin: 4000, tint: QuantumTheme.lavender)
                        temperatureButton(title: "Cool", kelvin: 6500, tint: QuantumTheme.sky)
                    }
                }
